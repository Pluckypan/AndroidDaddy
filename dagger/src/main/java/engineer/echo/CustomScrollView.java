...
public class CustomScrollView extends LinearLayout {
    //public static final int OVERSCROLL_DISTANCE = 150;
    ...
    private int overscrollDistance = 150;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (fScroller.springBack(x, y, 0, 0, 0, 0)) {
                    onSpringBack(x, y);
                    this.invalidate();
                }
    ...
    protected void onOverScrolled(int scrollX, int scrollY, boolean clampedX, boolean clampedY) {
        ...
        if (!fScroller.isFinished()) {
            ...
            if (clampedX || clampedY) {
                int x = this.getScrollX();
                int y = this.getScrollY();
                boolean back = fScroller.springBack(x, y, 0, 0, 0, 0);
                if (back) {
                    onSpringBack(x, y);
                }
            }
    ...
    private void onSpringBack(int x, int y) {
        Log.d("Plucky", "onSpringBack x:" + x + " y:" + y + " overscrollDistance:" + overscrollDistance);
        if (springBackListener != null) {
            springBackListener.onSpringBack(x, y);
        }
    }
    ...
    private SpringBackListener springBackListener;

    public void setSpringBackListener(SpringBackListener springBackListener) {
        this.springBackListener = springBackListener;
    }

    public interface SpringBackListener {
        void onSpringBack(int x, int y);
    }
}